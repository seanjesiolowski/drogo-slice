...
@pytest.mark.asyncio
async def test_low_stock_report_uses_storage_class_thresholds(client: AsyncClient):
    """Items with a storage class use its thresholds instead of project defaults.

    Default critical=0.5, low=0.99. With a SC of critical=0.3, low=0.6:
    - current=5, par=10 → 5 <= 10*0.3=3? No → 5 <= 10*0.6=6? Yes → "low"
    - Same quantity without SC → 5 <= 10*0.5=5? Yes → "critical"
    """
    sc = await client.post(
        "/api/storage-classes/",
        json={"name": "Loose Shelf", "low_threshold": 0.6, "critical_threshold": 0.3},
    )
    sc_id = sc.json()["id"]
    ...
    assert by_name["Item With SC"]["status"] == "low"
    assert by_name["Item Without SC"]["status"] == "critical"
